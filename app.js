...
BTN_COPY.addEventListener('click', (event) => {
  navigator.clipboard.writeText(RESULT.doc.getValue()).then(
    function () {
...
const flatten = (name, object) => {
  ...
  for (const key in object) {
    ...
    if (Array.isArray(object[key])) {
      let arrayType = `${getArrayType(key, object[key])}[]`
    ...
    // Child is Json
      ...
      if (isNaN(key)) {
        result[name] = { ...result[name], [key]: `I${toCamelCase(key)}` }
      }
